...
void Thermostat::device_info_web(JsonArray & root, uint8_t & part) {
    ...
    if (part == 0) {
        if (export_values_main(json)) {
            ...
            create_value_json(root, F_(clockoffset), nullptr, F_(clockoffset_), nullptr, json);
        ...
        if (export_values_hc(hc, json)) {
            ...
            create_value_json(root, F_(control), FPSTR(prefix_str), F_(control_), nullptr, json);
...
bool Thermostat::export_values_main(JsonObject & rootThermostat) {
    ...
    if (model == EMSdevice::EMS_DEVICE_FLAG_RC30_1) {
        ...
        // Language
        Helpers::json_enum(rootThermostat, F_(language), {F_(german), F_(dutch), F_(french), F_(italian)}, ibaLanguage_);

        // Offset clock
        if (Helpers::hasValue(ibaClockOffset_)) {
            rootThermostat[F_(clockoffset)] = ibaClockOffset_; // offset (in sec) to clock, 0xff=-1s, 0x02=2s
        }
    }
...
bool Thermostat::export_values_hc(std::shared_ptr<Thermostat::HeatingCircuit> hc, JsonObject & dataThermostat) {
    ...
    // Heating Type
    Helpers::json_enum(dataThermostat, F_(heatingtype), {F_(off), F_(radiator), F_(convector), F_(floor)}, hc->heatingtype);
    ...
    if (model == EMS_DEVICE_FLAG_RC35 || model == EMS_DEVICE_FLAG_RC30_1) {
        ...
        Helpers::json_enum(dataThermostat, F_(controlmode), {F_(off), F_(outdoor), F_(simple), F_(mpc), F_(room), F_(power), F_(const)}, hc->controlmode);
    }

    // control
    Helpers::json_enum(dataThermostat, F_(control), {F_(off), F_(rc20), F_(rc3x)}, hc->control);
...
void Thermostat::register_mqtt_ha_config() {
    ...
    doc[F_(uniq_id)] = F_(thermostat);
    doc[F_(ic)]      = F_(iconthermostat);

    char stat_t[128];
    snprintf_P(stat_t, sizeof(stat_t), PSTR("%s/%s"), Mqtt::base().c_str(), Fc_(thermostat_data));
    ...
    snprintf_P(&topic[0], topic.capacity() + 1, PSTR("%s%s/%s/%s"),Fc_(hasensor), Mqtt::base().c_str(), Fc_(thermostat), Fc_(config));
...
void Thermostat::register_mqtt_ha_config(uint8_t hc_num) {
    ...
    case EMS_DEVICE_FLAG_RC35:
        ...
        Mqtt::register_mqtt_ha_sensor(hc_name, nullptr, F_(controlmode_), device_type(), F_(controlmode), nullptr, nullptr);
        Mqtt::register_mqtt_ha_sensor(hc_name, nullptr, F_(control_), device_type(), F_(control), nullptr, nullptr);
        Mqtt::register_mqtt_ha_sensor(hc_name, nullptr, F_(program_), device_type(), F_(program), nullptr, nullptr);
        break;
...
void Thermostat::process_CRFMonitor(std::shared_ptr<const Telegram> telegram) {
    ...
    changed_ |= telegram->read_bitvalue(hc->mode_type, 2, 0);      // bit 0, modetype (0=off, 1=on)
...
void Thermostat::process_RC35Set(std::shared_ptr<const Telegram> telegram) {
    ...
    changed_ |= telegram->read_value(hc->control, 26);        // 0-off, 1-rc20, 2-rc3x
...
bool Thermostat::set_language(const char * value, const int8_t id) {
    uint8_t lg = 0;
    if (!Helpers::value2enum(value, lg, {F_(german), F_(dutch), F_(french), F_(italian)})) {
        LOG_WARNING(F_(invalid));
        return false;
    }
...
bool Thermostat::set_control(const char * value, const int8_t id) {
    uint8_t ctrl = 0;
    if (!Helpers::value2enum(value, ctrl, {F_(off), F_(rc20), F_(rc3x)})) {
        LOG_WARNING(F_(invalid));
        return false;
    }
...
bool Thermostat::set_holiday(const char * value, const int8_t id) {
    ...
    std::shared_ptr<Thermostat::HeatingCircuit> hc     = heating_circuit(hc_num);
    if (hc == nullptr) {
        LOG_WARNING(F_(hc_not_found), hc_num, device_id());
        return false;
    }
